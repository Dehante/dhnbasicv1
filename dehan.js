...
const { DehanJson, DehanText, DehanBase, DehanBuffer, DehanUpImg } = require('./lib/fetcher')
const { recognize } = require('./lib/ocr')
const fontPath = ('./lib/Zahraaa.ttf')
const path = require('path')
const { exec, spawn } = require("child_process")
const { wait, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./lib/functions')
...
╰──────────────────`
				let buff = await DehanBuffer(ppimg)
...
〴 Ig owner: www.instagram.com/${imgeh}
〴 Gopay: ${owner} A/n ${ownerName}
━━━━━━━━━━━━━━━━━━━━
ぐ ⎝ INGFO BOT ⎞
〴 Nama Bot: ${me.name}
...
ぐ Ketik: ${prefix}owner untuk menghubungi admin bot kami.`
					buffer = await DehanBuffer(me.imgUrl)
		...
		case 'blocklist': 
			if (!isOwner) return dhnjing(ind.ownerb())
					...
					for (let block of blocked) {
						teks += `ぐ [${no.toString()}] @${block.split('@')[0]}\n`
					}
		...
		case 'tiktokstalk':				
				...
				try {
						...
						buffer = await DehanBuffer(user.avatarLarger)
		...
		case 'gc':
		  if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
            ...
            case 'owner':
            case 'developer':
                  dehan.sendMessage(from, {displayname: `${ownerName}`, vcard: vcard}, MessageType.contact, { quoted: dhn})
  ...
  case 'asupan':
      ...
      if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
      		...
      		dhnjing(ind.wait(pushname))
                ...
                buffer = await DehanBuffer(randKey.result)
   ...
   if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
				...
				if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
					...
					if ((isMedia && !dhn.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						dhnjing(`Kirim gambar dengan caption ${prefix}sticker atau reply/tag gambar`)
					}
					break
		case 'fitnah':
                 		if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
			...
			case 'infogroup':
			    ...
			    buffer = await DehanBuffer(ppUrl)
...
break
 	...
 	if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
					  		  if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
			      dhnjing(ind.wait(pushname))
				if (!isQuotedSticker) return dhnjing('*reply/Tag sticker!*')
...
ぐ ALL USER ${name}`
		...
		if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
			  if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
			   ...
			   if (!isOwner) return dhnjing(ind.ownerb())
		ApiKey = args[0]
                    get_result = await DehanJson(`http://lolhuman.herokuapp.com/api/checkapikey?apikey=${ApiKey}`)
		...
		if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
			  if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
			   ...
			   if (!isOwner) return dhnjing(ind.ownerb())
		ApiKey = args[0]
                    get_result = await DehanJson(`https://api.xteam.xyz/cekey?APIKEY=${ApiKey}`)
			...
			break 
				...
				case 'totaluser':
					if (isBanned) return dhnjing('Maaf kamu sudah terbenned!')
					  		  if (!isRegistered) return dhnjing( ind.noregis(prefix, pushname))
					me = dehan.user
					buff = await DehanBuffer(me.imgUrl)
...
break
    	...
    	case 'addstik':
	    case 'addstick':
		if (!isQuotedSticker) return dhnjing('reply stiker nya')
		if (!isPremium) return dhnjing('Maaf kamu bukan user premium!')
		svst = args.join(" ")		
		...
		break
        case 'addvn':
		if (!isQuotedAudio) return dhnjing('reply vnnya')
	    if (!isPremium) return dhnjing('Maaf kamu bukan user premium!')
		svst = args.join(" ")				
        ...
        case 'addimg':
	    case 'addimage':
		if (!isQuotedImage) return dhnjing('reply gambarnya')
		if (!isPremium) return dhnjing('Maaf kamu bukan user premium!')
		svst = args.join(" ")		
		...
		if (!isQuotedVideo) return dhnjing('reply videonya')
		svst = args.join(" ")		
